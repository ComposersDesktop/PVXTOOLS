...
void fft_(float* anal,float*  banal, int n, int N2, int nspn, int isn);
...
/*  allocate workspace - assume no errors! */
    ...
    state->at = (float *) calloc(maxf,sizeof(float));
    state->ck = (float *) calloc(maxf,sizeof(float));
    state->bt = (float *) calloc(maxf,sizeof(float));
    state->sk = (float *) calloc(maxf,sizeof(float));
    state->np = (int *) calloc(maxp,sizeof(int));
